...
const AuthModal = () => {
  ...
  const heading = isForgot ? 'Reset your password' : isLogin ? 'Welcome back' : 'Create your account'
  const subheading = isForgot
    ? 'Enter your email and we will send you a reset link.'
    : isLogin
      ? 'Sign in to manage your uploads.'
      : 'Sign up to keep track of everything you share.'
        ...
        <Modal.Container placement="center" size="md">
          <Modal.Dialog className="bg-surface rounded-2xl p-6 max-w-md w-full font-sans">
            ...
            <Modal.Header className="px-0 pt-0 pb-4">
              <Modal.Heading className="text-2xl font-serif text-[var(--ink-900)]">{heading}</Modal.Heading>
              <p className="text-sm text-[var(--ink-600)] mt-1">{subheading}</p>
            </Modal.Header>
            ...
            <Modal.Body className="px-0 pb-0 flex flex-col gap-5">
              {confirmationEmail ? (
                ...
                <>
                  {isForgot ? (
                    <ForgotPasswordForm onBackToSignIn={() => switchMode('login')} />
                  ) : (
                    <>
                      ...
                      {isLogin ? (
                        <form key="login-form" onSubmit={loginForm.handleSubmit(onLogin)} className="flex flex-col gap-4">
                          ...
                          <div className="flex justify-end -mt-1">
                            <button type="button" onClick={() => switchMode('forgot')} className="text-sm text-[var(--brand-400)] hover:underline">
                              Forgot password?
                            </button>
                          </div>
